...
{
    [Theory]
    [InlineData(new byte[] { 2 }, 2)]          // exact match
    [InlineData(new byte[] { 3, 2 }, 2)]       // client offers extra unknown version + 2
    [InlineData(new byte[] { 9, 8, 2 }, 2)]    // 2 is the only common one
    [InlineData(new byte[] { 3, 4 }, 0)]       // no overlap
    [InlineData(new byte[] { 1 }, 0)]          // v1 is no longer supported
    ...
    [Fact]
    public void HappyPath_NegotiatesVersionTwo()
    {
        ...
        Assert.Equal(2, sh.NegotiatedVersion);
